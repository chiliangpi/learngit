...
df_org = spark.read.format('csv').load('hdfs://10.0.4.217:8020/crawler/' + fileName, schema=schema)\
              .select('fn', 'fc', 'org', 'dst', 'departDate', 'id', 'queryDate', 'price', 'orgPrice','futureMinPrice', 'dayofweek', 'distance', 'seatLeft', 'departTime','isShare', 'trend')\
...
def write_DCN_featureDict(df_shuffled, params):
...
write_DCN_featureDict(df_shuffled, params)
logger.info("====deep&cross featureDict finished====")


df_train_DNN = df_shuffled.filter(df.departDate < params["train_val_splitDate"])
df_val_DNN = df_shuffled.filter(df.departDate >= params["train_val_splitDate"])
df_val_DNN.cache()
df_train_DNN_sample = df_train_DNN.drop(*list(set(params['dropFeatures'])-set(params['baseColumns']))).sample(False, float("%.4f" % (3e5/df_train_DNN.count())))
...
df_shuffled.drop(*list(set(params['dropFeatures'])-set(params['baseColumns']))).write.format('parquet').save("hdfs://10.0.4.217:8020/crawler/" + featureData_fileName)
df_train_DNN.drop(*list(set(params['dropFeatures'])-set(params['baseColumns']))).write.format('parquet').save("hdfs://10.0.4.217:8020/crawler/" + DNN_trainData_fileName)
df_train_DNN_sample.repartition(1).drop(*list(set(params['dropFeatures'])-set(params['baseColumns']))).write.format('parquet').save("hdfs://10.0.4.217:8020/crawler/" + DNN_trainSampleData_fileName)
...
for day in ['2019-07-25', '2019-07-26', '2019-07-27', '2019-07-28', '2019-07-29']:
    ...
    df_val_DNN.filter(col('queryDate') == day).repartition(1).drop(*list(set(params['dropFeatures'])-set(params['baseColumns']))).write.format('parquet').save(
    ...
    df_val_DNN.filter(col('departDate')== day).repartition(1).drop(*list(set(params['dropFeatures'])-set(params['baseColumns']))).write.format('parquet').save(
